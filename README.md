...
We will run your submissions on an Amazon EC2, which has a 1Ghz processor equivalent and 200mb of RAM. See below for more details.
...
The actual competitions will be run using nightly automated tournaments on an Amazon EC2 like cluster (1 Ghz / 200MB RAM machines). See the submission instructions for details of how to enter a team into the tournaments via the main branch. 

We will try to run **tournaments frequently**, if possible every day. The number of games each team plays every other team will depend on the number of teams. To reduce randomness, each team will play each other 3 times. 

The **layouts** used in the tournament will be drawn from randomly generated layouts. All layouts are symmetric, and the team that moves first is randomly chosen. 

The **results** of each tournament (pre-contest, preliminary, and final) will be published on a web-page where you will be able to see the overall rankings and scores for each match. You can also download replays, the layouts used, and the stdout / stderr logs for each agent. So be careful not to leak your tactics or sensitive information to tother teams via printing to the console.
...
* If you want to use any other 3rd-party executable please discuss with us before submission. You can assume that `TensorFlow`, `keras`, `sklearn`, `numpy`, `scipy` and `neat-python` libraries are installed in our running environment, using the latest version available in Ubuntu 20.04. 

## Fair play

* Submit your own original work.
* Report suspected violations.
* Not engage in any other activities that will dishonestly improve your results or dishonestly improve or damage the results of others.

Being a contest, we expect **fair play** of all teams in this project. If you are in doubt of whether something would break the good spirit of the project. Any behaviour or code providing an unfair advantage or causing harm will be treated very seriously. We trust you, do not let us down and be a fair player.
